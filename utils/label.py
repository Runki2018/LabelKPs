...
class label_it:
    ...
    def __init__(self, annotation_file):
        ...
        self.new_ann_file = save_dir + time.strftime("%Y-%m-%d", time.localtime()) + ".json"
    ...
    def get_raw_keypoints(self):
        """获取粗略的关键点坐标"""
        # 深拷贝，不然pop后就修改了原标注信息
        keypoints = self.annotations_list[self.index]["keypoints"].copy()  
        pop_i = 0  # 去除每个关键点的置信度
        for k in range(21):
            pop_i += 2
            keypoints.pop(pop_i)
    ...
    def save_annotations(self, is_override=False):
        """保存当前所有图像的修改"""
        ...
        if is_override:
            json.dump(self.json_ann, open(self.ann_file, "w"), indent=4)  # 覆盖原标注文件
        else:
            json.dump(self.json_ann, open(self.new_ann_file, "w"), indent=4)  # 新建一个标注文件，并以当前日期命名
    ...
    def get_classes(self):
        ...
        """
        class_id = self.images_list[self.index]["label"]  # int 0~8
        if "predict" in self.images_list[self.index].keys():
            predict_id = self.images_list[self.index]["predict"]
        else:
            predict_id = class_id
        return class_id, self.classes[predict_id]

    def set_classes(self, label: int):
        """设置类别标签，用于修改错误的类别标签"""
        self.images_list[self.index]["label"] = label
