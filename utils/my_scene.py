...
class MyScene(QGraphicsScene):
    ...
    wh_points = 10  # 关键点大小的常量一直保存不变
    wh_current_points = 10  # 当前关键点的大小
    ...
    def __init__(self, list_widget):  # 初始函数
        super(MyScene, self).__init__()  # 实例化QGraphicsScene
        self.listWidget = list_widget  # 列表控件，用于显示21个关键点坐标
    ...
    def init_scene(self, img_path, keypoints):
        ...
        if self.points_list:
            ...
            self.wh_current_points = self.wh_points
            # for item in self.items():
            #     self.removeItem(item)
            self.clear()
        # 2、读入图片，并将图片的大小设置为scene的大小
        # 单独存储原始图像，用于后面的缩放保持质量，每次缩放基于原图。
    ...
    def center2LeftTop(self, x, y):
        ...
        lx = x - self.wh_current_points / 2   # 计算左上角点坐标
        ly = y - self.wh_current_points / 2
    ...
    def leftTop2Center(self, lx, ly):
        ...
        x, y = lx + self.wh_current_points / 2, ly + self.wh_current_points / 2
        return x, y
    ...
    def addPoint(self, index, x, y):
        ...
        self.current_point = self.addEllipse(lx, ly, self.wh_current_points, self.wh_current_points, self.pen, brush)
    ...
    def mousePressEvent(self, event):  # 重载鼠标事件
        ...
        if event.button() == Qt.LeftButton and not self.acceptMove:  # 仅左键事件触发且不接受拖拽时
            ...
            if isinstance(self.current_point, QGraphicsEllipseItem):
                self.wh_current_points *= 2  # 选中后放大关键点
                lx, ly = self.center2LeftTop(x, y)
                self.current_point.setRect(QRect(lx, ly, self.wh_current_points, self.wh_current_points))  # 生效
        ...
        if event.button() == Qt.RightButton and self.acceptMove:  # 仅右键事件触发
            ...
            if isinstance(self.current_point, QGraphicsEllipseItem):
                ...
                self.wh_current_points = self.wh_points  # 释放后缩小关键点
                lx, ly = self.center2LeftTop(x, y)
                self.current_point.setRect(QRect(lx, ly, self.wh_current_points, self.wh_current_points))
    ...
    def update_ItemPos(self):
        ...
        for i, point in enumerate(self.points_list):
            ...
            lx, ly = self.center2LeftTop(x, y)
            w, h = self.wh_current_points, self.wh_current_points
